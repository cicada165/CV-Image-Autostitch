...
def computeSphericalWarpMappings(dstShape, f, k1, k2):
    ...
    vec = np.zeros(3)
    vec[0] = np.sin(0.0) * np.cos(0.0) #0
    vec[1] = np.sin(0.0) #0
    vec[2] = np.cos(0.0) * np.cos(0.0) #1
    min_y = vec[1]
    ...
    #spherial coordinates
    xhat = np.sin(xf) * np.cos(yf) 
    yhat = np.sin(yf)

    xprj = xhat / (np.cos(xf) * np.cos(yf)) 
    yprj = yhat / (np.cos(xf) * np.cos(yf)) 
    
    r2 = xprj**2 + yprj**2

    xt = xprj * (one + k1*r2 + k2*(r2**2))
    yt = yprj * (one + k1*r2 + k2*(r2**2))
