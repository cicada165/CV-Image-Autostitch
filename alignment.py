...
def computeHomography(f1, f2, matches, A_out=None):
    ...
    #print('A_matrix_shape',A_matrix_shape)
...
def alignPair(f1, f2, matches, m, nRANSAC, RANSACthresh):
    ...
    for i in range(nRANSAC):
        H = np.eye(3)
        selected = np.random.choice(matches, sampleNumber)
        ...
        if(m == eTranslate):
            ...
            H = computeHomography(f1, f2, selected)

        inliners = getInliers(f1, f2, matches, H, RANSACthresh)
        
        if(len(inliners) > maxNumber):
            maxNumber = len(inliners)
    ...
    #TODO-BLOCK-END
    #END TODO
    M = leastSquaresFit(f1, f2, matches, m, maxInliners )
    return M
...
def getInliers(f1, f2, matches, M, RANSACthresh):
    ...
    for i in range(len(matches)):
        ...
        (x2, y2) = f2[matches[i].queryIdx].pt
        ...
        distance = np.sqrt((x3-x2)**2 + (y3-y2)**2)
...
def leastSquaresFit(f1, f2, matches, m, inlier_indices):
    ...
    if m == eTranslate:
        ...
        for i in range(len(inlier_indices)):
            ...
            (x2, y2) = f2[matches[index].queryIdx].pt
