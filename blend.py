...
def imageBoundingBox(img, M):
    ...
    #transform 4 corners
    t1 = np.dot(M, [0,0,1]) 
    t2 = np.dot(M, [width-1, 0, 1])
    t3 = np.dot(M, [0, height-1,1])
    t4 = np.dot(M, [width-1, height-1,1])
    ...
    minX = min(t1[0], t2[0], t3[0], t4[0])
    maxX = max(t1[0], t2[0], t3[0], t4[0])

    minY = min(t1[1], t2[1], t3[1], t4[1])
    maxY = max(t1[1], t2[1], t3[1], t4[1])
...
def accumulateBlend(img, acc, M, blendWidth):
    ...
    for i in range(c):
        ...
        img[:,:, i] = ((img[:, :, i] - cmin).astype(float) * 255 / (cmax - cmin)).astype(int)

    weights = np.ones((h,w))
    for i in range(blendWidth):
        weights[:, i] = [float(c) / blendWidth] * h
        weights[:, w - i - 1] = [float(c) / blendWidth] * h

    feathering = np.zeros((h,w,c+1))
    for i in range(c):
        feathering[:,:, i] = img[:,:, i] * weights
    feathering[:,:, -1] = weights
    
    addon = np.zeros(acc.shape)
    for i in range(c+1):
        addon[:,:, i] = cv2.warpPerspective(feathering[:,:, i], M, dsize = (acc.shape[1],acc.shape[0]), flags = cv2.INTER_LINEAR)
...
def getAccSize(ipv):
    ...
    for i in ipv:
        ...
        boundBoxMinX, boundBoxMinY, boundBoxMaxX, boundBoxMaxY = imageBoundingBox(img, M)

        minX = min(boundBoxMinX, minX)
        minY = min(boundBoxMinY, minY)
        maxX = max(boundBoxMaxX, maxX)
        maxY = max(boundBoxMaxY, maxY)
